...
def reducer(sequence):
    ...
    for key, group in groupby(sequence, lambda x: x[0]):
        result.append(
            (key, sum(value for _, value in group))  # a
        )  # group= [(a,1), (a, 1)...] Se hace la suma de todas las a
    return result
...
def create_marker(
    output_directory,
):  # Es una funcion que crea el marcador para ver si existe o no el archivo
    """Create Marker"""
